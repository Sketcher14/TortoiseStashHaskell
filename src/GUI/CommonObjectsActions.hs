...
arrowButtonClick :: IORef DataState -> IORef CurrentArrow -> Int -> (DataState -> Int -> File) -> Bool -> Dialog -> Entry -> IO ()
arrowButtonClick refState refCurrentArrow id getFileFromDataStateFun isEnc dFileSave dFileSaveEntry = do
...
onArrowButtonsClick :: IORef DataState -> IORef CurrentArrow ->
  (DataState -> Int -> File) -> Bool -> Dialog -> Entry -> ButtonsPack -> IO ()
onArrowButtonsClick
  refState
  refCurrentArrow
  ...
  ButtonsPack { but1 = arrowBut1, but2 = arrowBut2, but3 = arrowBut3, but4 = arrowBut4, but5 = arrowBut5 } = do
    on arrowBut1 buttonActivated $
      arrowButtonClick refState refCurrentArrow 1 getFileFromDataStateFun isEncryption dFileSave dFileSaveEntry
    on arrowBut2 buttonActivated $
      arrowButtonClick refState refCurrentArrow 2 getFileFromDataStateFun isEncryption dFileSave dFileSaveEntry
    on arrowBut3 buttonActivated $
      arrowButtonClick refState refCurrentArrow 3 getFileFromDataStateFun isEncryption dFileSave dFileSaveEntry
    on arrowBut4 buttonActivated $
      arrowButtonClick refState refCurrentArrow 4 getFileFromDataStateFun isEncryption dFileSave dFileSaveEntry
    on arrowBut5 buttonActivated $
      arrowButtonClick refState refCurrentArrow 5 getFileFromDataStateFun isEncryption dFileSave dFileSaveEntry
    return ()